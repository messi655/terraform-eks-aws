  ...
  worker_groups = [
    {
      ...
      additional_userdata  = "echo \"* - nofile 65536\"  >> /etc/security/limits.conf"
      subnets              = "${join(",", var.private_subnets)}"
      asg_desired_capacity = "2"
      asg_min_size         = "2"
      asg_max_size         = "6"
      root_volume_size     = "100"
      name                 = "staging_in_app_avay"
      key_name             = "staging-prod"
      ami_id               = "${var.ami_id}"
      autoscaling_enabled  = true
      protect_from_scale_in = true
      kubelet_extra_args   = "--node-labels=workergroup=prod_avay"
    },
    {
      # This will launch an autoscaling group with only On-Demand instances
      ...
      name                 = "staging_in_app_log_monitoring"
      key_name             = "staging-prod"
      ami_id               = "${var.ami_id}"
      autoscaling_enabled  = true
      protect_from_scale_in = true
      kubelet_extra_args   = "--node-labels=workergroup=logging"
    },
  ]
